...
class CameraExamStartView extends ConsumerWidget {
  const CameraExamStartView({super.key, this.onTakeShoot});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return MyUI(
      child: Scaffold(
        appBar: AppBar(title: const Text('Ambil Foto Mulai Ujian')),
        body: Stack(
          children: [
            CameraView(
              ...
              onTakeShoot: (file) async => await ref.read(examCtrlProvider).updatePhotoExamStart(file),
            ),
          ],
        ),
      ),
