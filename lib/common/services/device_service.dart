...
class DeviceService {
  ...
  final _kLogName = 'DEVICE-SERVICE';

  Future<Map<String, dynamic>> getDeviceInfo({bool showLog = false}) async {
    ...
    try {
      if (kIsWeb) {
        ...
        log('DeviceId : $deviceId', name: _kLogName);
        log('DeviceName : $deviceName', name: _kLogName);
        ref.read(deviceIdProvider.notifier).state = deviceId;
        ref.read(deviceNameProvider.notifier).state = deviceName;
        return result;
      } else {
